...
export async function listActionTasks(status?: string) {
  const filter = status ? { status } : {};
  return ActionTaskModel.find(filter).sort({ createdAt: -1 }).limit(200);
}

export async function listIssues(status?: string) {
  const filter = status ? { status } : {};
  return IssueModel.find(filter).sort({ createdAt: -1 }).limit(200);
}
